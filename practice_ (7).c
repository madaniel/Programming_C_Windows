...
/*																*/					
/*																*/					
/*					התוכנית מבצעת מספר פעולות					*/
/*		מציירת מלבן,מזהה משולשים,סופרת תווים,מגלה מסםר מקסימלי	*/
/*						ומדפיסה את לוח הכפל						*/						 										
/*																*/																									
/*																*/					
/*																*/					
			...
			while (multi_value>20)
				{
					...
					scanf("%d",&multi_value);//וידוא שהערך לא גדול מ-20
				}
			system ("cls");
			for (j=1;j<=multi_value;j++)//לולאת השורות
				{
					for (i=1;i<=multi_value;i++)
						{
							printf("%4d",multi_number);//לולאת העמודות
							multi_number+=j;
						}
					multi_number=j+1;//הערך הראשוני תמיד שווה למספר השורה
					printf("\n");
				}
			printf("\n\nPress Any Key\n");
			multi_number=1;//איפוס ערכים לשימוש חוזר
			getch();
			break;
